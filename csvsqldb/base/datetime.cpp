...
DateTime::DateTime(UInt16 year, UInt16 month, UInt16 day, UInt16 hour, UInt16 minute, UInt16 second, TimeScope ts)
: _timeZone(TimeZone::defaultTimeZone())
{
  if (ts == LOCAL) {
    Time mod;
...
DateTime::DateTime(UInt16 year, UInt16 month, UInt16 day, UInt16 hour, UInt16 minute, UInt16 second, const TimeZone& timeZone)
: _timeZone(timeZone)
{
  Time mod;
  Date d(year, month, day);
