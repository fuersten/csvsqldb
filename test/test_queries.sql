...
explain ast select emp_no,(first_name || ' ' || last_name) as name,( (CURRENT_DATE - birth_date)/365 ) as dt from employees where birth_date > DATE'1965-01-01' order by dt;

explain exec select emp_no,(first_name || ' ' || last_name) as name,( (CURRENT_DATE - birth_date)/365 ) as dt from employees where birth_date > DATE'1965-01-01' order by dt;

select emp_no,(first_name || ' ' || last_name) as name,(CURRENT_DATE - birth_date)/365 as dt from employees where last_name = 'Schmiedel';

select emp_no,first_name || ' ' || last_name as name,gender,extract(year from birth_date) from employees where extract(year from birth_date) >= 1965;

select emp_no,(first_name || ' ' || last_name) as name,CAST(CURRENT_DATE - birth_date as REAL)/365.0 as dt from employees where last_name = 'Schmiedel';

select emp_no,first_name || ' ' || last_name as name,(CURRENT_DATE - birth_date)/365 as dt,CURRENT_DATE from employees where last_name = 'Schmiedel' and emp_no > 490000;
...
SELECT emp.first_name,emp.last_name,dept_emp.dept_no,departments.dept_name FROM employees as emp JOIN dept_emp ON emp.emp_no = dept_emp.emp_no JOIN departments ON dept_emp.dept_no = departments.dept_no;
