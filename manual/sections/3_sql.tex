...
In this syntax diagrams a terminal symbol is represented by \tikz \node[scale=0.75] [terminal] {SELECT}; and a non-terminal by \tikz \node[scale=0.75] [nonterminal] {IDENTIFIER};.
...
\graph [use existing nodes] {
  p1 -> select -- p3 -> select list -> from -> table reference -> table expressions -> p8;
  p2 ->[vh path] distinct ->[hv path] p3;
  p2 ->[vh path] all ->[hv path] p3;
  p6 ->[vh path] comma ->[hv path] p5;
};
\putendline{p8}
\end{SyntaxGrammar}
...
\graph [use existing nodes] {
  p1 -- p2 -> asterisk -> p7;
  p2 ->[vh path] expression ->[hv path] p5;
  p3 ->[vh path] identifier ->[hv path] p5;
  p3 ->[vh path] as ->[hv path] p4;
  p5 ->[vh path] comma ->[hv path] p2;
};
\putendline{p7}
\end{SyntaxGrammar}
...
\graph [use existing nodes] {
  p1 -> tableorsubselect -> p6;
  p2 ->[vh path] join type -> tableorsubselect2 -> on -> expression ->[hv path] p5;
