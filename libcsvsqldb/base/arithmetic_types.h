...
template<typename T>
T absT( T v ) { return v<0 ? -v : v; }

template<typename T>
int signT( T v ) { return v<0 ? -1 : 1; }
...
******************************************************************************/
template <int N, typename T=int> struct exp_10    { static const T value= exp_10<N-1,T>::value * 10; };
template <>      struct exp_10<0,int> { static const int value = 1; };
template <>      struct exp_10<0,mpf::Int64> { static const mpf::Int64 value = 1; };
...
******************************************************************************/
template <int N=2, typename T=int> class fixed_decimal : public arithmetic_operator_helper< fixed_decimal<N,T> >
{
public:
    typedef T rep_t;
    ...
    explicit fixed_decimal(int m=0, int n=0) : rep_((static_cast<T>(m)*scale_+(n % scale_)*signT(m)))     {}
    explicit fixed_decimal(unsigned int u)   : rep_((u*scale_))                      {} 
    explicit fixed_decimal(unsigned long ul) : rep_((ul*scale_))                     {}
    explicit fixed_decimal(double f)         : rep_((T)(f*scale_ + 0.5))             {}
