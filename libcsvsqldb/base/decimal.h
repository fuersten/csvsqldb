...
namespace mpf
{
  ...
  public:
    ...
    Decimal(const Decimal& ref)
    : _val(ref._val)
    {
    }
    explicit Decimal(int m, int n = 0)
    : _val(m, n)
    {
    }
    ...
    explicit Decimal(const mpf::Int64& val)
    : _val(val)
    {
    }
    ...
    {
      return double(_val.rep()) / _val.scale();
    }
    ...
    mpf::Int64 characteristic() const
    {
      return _val.characteristic();
    }
  ...
  private:
    Decimal(fixed_decimal<S, mpf::Int64> internal)
    : _val(internal)
    {
    }
